...
if (!$httpRequestChecker->checkHttpRequest($request)) {
...
$holder->add(
    new Step\PrepareNewestVersionStep(
        new NewestVersionChecker($clientFactory, new DomainProvider($bootstrap->getEnvironment())),
        new NewestInstancePreparator(),
        new GuzzleDownloader($clientFactory)
    )
);
