...
class ContainersRestController extends RestController
{
     ...
     * @Rest\View(
     *      serializerEnableMaxDepthChecks="true",
     *      serializerGroups={"containers", "list_view", "permissions"}
     * )
     *
    ...
    {
        $repository = $this
            ->get('seven_tag_container.repository.container_repository');

        return new RestView\ListView(
            $repository->findByCredentialsWithLimitAndOffset(
                $this->getUser(),
                $paramFetcher->get('limit'),
                $paramFetcher->get('offset')
            ),
            $repository->countByCredentials($this->getUser())
        );
    }
