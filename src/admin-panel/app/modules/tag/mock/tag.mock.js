 ...
 */
var tagMock = ($httpBackend, ngTableParams, $filter, Mock) => {

    var id = Mock.highestId('tags') + 1;
    ...
    $httpBackend.whenGET(/\/api\/containers\/(.*)\/tags\?*/).respond((method, url, data) => {
        ...
        var tags = Mock.query('tags', criteria);

        var tagsIndex = tags.length;

        while (tagsIndex--) {

            var triggersIndex = tags[tagsIndex].triggers.length;

            while (triggersIndex--) {

                var trigger = Mock.get('triggers', tags[tagsIndex].triggers[triggersIndex].id);
