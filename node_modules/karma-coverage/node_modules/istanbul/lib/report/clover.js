...
function addClassStats(node, nodeAbsolutePath, fileCoverage, writer) {
...
function walk(node, collector, writer, level, projectRoot) {
    ...
    if (node.packageMetrics) {
        ...
        node.children.filter(function (child) { return child.kind !== 'dir'; }).
            forEach(function (child) {
                var nodeAbsolutePath = child.fullPath();
                addClassStats(child, nodeAbsolutePath, collector.fileCoverageFor(child.fullPath()), writer);
            });
        writer.println('\t\t</package>');
    }
    node.children.filter(function (child) { return child.kind === 'dir'; }).
...
Report.mix(CloverReport, {
    ...
    writeReport: function (collector, sync) {
        var summarizer = new TreeSummarizer(),
            ...
            projectRoot = this.opts.basePath !== undefined ? path.join(this.projectRoot, this.opts.basePath) : this.projectRoot,
            that = this,
            tree,
            root;

        collector = collector.reportView();
