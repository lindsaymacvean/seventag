...
class TriggerRepository extends EntityRepository implements VersionableRepositoryInterface
{
     ...
     */
    public function findByContainer(Container $container, $limit = 20, $offset = 0, array $excludeTriggers = [])
    {
     ...
     * @return int
     */
    public function countByContainer(Container $container, array $excludeTriggers = [])
    {
        $queryBuilder = $this->getBaseQuery(true)
            ->select('count(t.id)')
            ->andWhere('t.container = :container')
